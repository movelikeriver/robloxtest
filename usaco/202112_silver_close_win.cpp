...
// trick #1:
// f1, p, ..., p, f2
//       [  ^   ]    ---> the moving window length is half of f2-f1
// so, we can get `first_v`.
//
// trick #2:
// f1, p, ..., p, f2
//    ^         ^    ---> total_sum
// if put two points, anyway all `p`s between f1 and f2 can be collected.
// so, the incremental value of adding 2nd point is `second_v` = total_sum - `first_v`
// meaning `second_v` and `first_v` has become independent.
//
// sort all elements of `first_v` and `second_v`, pick up the top N.
